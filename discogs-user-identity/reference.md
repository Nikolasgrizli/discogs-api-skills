...
### GET /oauth/identity
Retrieve basic information about the authenticated user. You can use this resource to find out who you’re authenticated as, and it also doubles as a good sanity check to ensure that you’re using OAuth correctly. For more detailed information, make another request for the user’s Profile.
...
### GET /users/{username}
Retrieve a user by username. If authenticated as the requested user, the email key will be visible, and the num_list count will include the user’s private lists. If authenticated as the requested user or the user’s collection/wantlist is public, the num_collection / num_wantlist keys will be visible.

**Parameters:**
- `username` (string, required) — The username of whose profile you are requesting. _(e.g. `rodneyfool`)_
...
**Parameters:**
- `username` (string, required) — The username of the user. _(e.g. `vreon`)_
- `name` (string, optional) — The real name of the user. _(e.g. `Nicolas Cage`)_
- `home_page` (string, optional) — The user’s website. _(e.g. `www.discogs.com`)_
- `location` (string, optional) — The geographical location of the user. _(e.g. `Portland`)_
- `profile` (string, optional) — Biographical information about the user. _(e.g. `I am a Discogs user!`)_
- `curr_abbr` (string, optional) — Currency for marketplace data. Must be one of the following: USD GBP EUR CAD AUD JPY CHF MXN BRL NZD SEK ZAR _(e.g. `USD`)_
...
**Parameters:**
- `username` (string, required) — The username of the submissions you are trying to fetch. _(e.g. `shooezgirl`)_
...
Retrieve a user’s contributions by username. Accepts Pagination parameters. Valid sort keys are: label artist title catno format rating year added Valid sort_order keys are: asc desc
