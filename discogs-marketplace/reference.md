...
## Inventory
Returns the list of listings in a user’s inventory. Accepts Pagination parameters. Basic information about each listing and the corresponding release is provided, suitable for display in a list. For detailed information about the release, make another API call to fetch the corresponding Release.
...
**Parameters:**
- `username` (string, required) — The username for whose inventory you are fetching _(e.g. `360vinyl`)_
- `status` (string, optional) — Only show items with this status. _(e.g. `for sale`)_
- `sort` (string, optional) — Sort items by this field: listed price item (i.e. the title of the release) artist label catno audio status (when authenticated as the inventory owner) location (when authenticated as the inventory owner) _(e.g. `price`)_
...
**Parameters:**
- `listing_id` (number, required) — The ID of the listing you are fetching _(e.g. `172723812`)_
...
Edit the data associated with a listing. If the listing’s status is not For Sale, Draft, or Expired, it cannot be modified – only deleted. To re-list a Sold listing, a new listing must be created. Authentication as the listing owner is required.

**Parameters:**
- `listing_id` (number, required) — The ID of the listing you are fetching _(e.g. `172723812`)_
- `release_id` (number, required) — The ID of the release you are posting _(e.g. `1`)_
- `condition` (string, required) — The condition of the release you are posting. Must be one of the following: Mint (M) Near Mint (NM or M-) Very Good Plus (VG+) Very Good (VG) Good Plus (G+) Good (G) Fair (F) Poor (P) _(e.g. `Mint`)_
- `sleeve_condition` (string, optional) — The condition of the sleeve of the item you are posting. Must be one of the following: Mint (M) Near Mint (NM or M-) Very Good Plus (VG+) Very Good (VG) Good Plus (G+) Good (G) Fair (F) Poor (P) Generic Not Graded No Cover _(e.g. `Fair`)_
- `price` (number, required) — The price of the item (in the seller’s currency). _(e.g. `10.00`)_
- `comments` (string, optional) — Any remarks about the item that will be displayed to buyers. _(e.g. `This item is wonderful`)_
- `allow_offers` (boolean, optional) — Whether or not to allow buyers to make offers on the item. Defaults to false. _(e.g. `true`)_
- `status` (string, required) — The status of the listing. Defaults to For Sale. Options are For Sale (the listing is ready to be shown on the Marketplace) and Draft (the listing is not ready for public display). _(e.g. `Draft`)_
- `external_id` (string, optional) — A freeform field that can be used for the seller’s own reference. Information stored here will not be displayed to anyone other than the seller. This field is called “Private Comments” on the Discogs website. _(e.g. `10.00`)_
- `location` (string, optional) — A freeform field that is intended to help identify an item’s physical storage location. Information stored here will not be displayed to anyone other than the seller. This field will be visible on the inventory management page and will be available in inventory exports via the website. _(e.g. `10.00`)_
- `weight` (number, optional) — The weight, in grams, of this listing, for the purpose of calculating shipping. Set this field to auto to have the weight automatically estimated for you. _(e.g. `10.00`)_
- `format_quantity` (number, optional) — The number of items this listing counts as, for the purpose of calculating shipping. This field is called “Counts As” on the Discogs website. Set this field to auto to have the quantity automatically estimated for you. _(e.g. `10.00`)_
...
**Parameters:**
- `order_id` (number, required) — The ID of the order you are fetching _(e.g. `1-1`)_
...
**Parameters:**
- `order_id` (number, required) — The ID of the order you are fetching _(e.g. `1-1`)_
- `status` (string, optional) — The status of the Order you are updating. Must be one of the following: New Order Buyer Contacted Invoice Sent Payment Pending Payment Received In Progress Shipped Refund Sent Cancelled (Non-Paying Buyer) Cancelled (Item Unavailable) Cancelled (Per Buyer's Request) the order’s current status _(e.g. `New Order`)_
- `shipping` (number, optional) — The order shipping amount. As a side-effect of setting this value, the buyer is invoiced and the order status is set to Invoice Sent. _(e.g. `5.00`)_
- `tracking` (object, required) — Set tracking information for the order. Only the seller can set tracking information; buyers will receive a 403 Forbidden response. The tracking object has the following properties:
...
**Parameters:**
- `status` (string, optional) — Only show orders with this status. Valid status keys are: All New Order Buyer Contacted Invoice Sent Payment Pending Payment Received In Progress Shipped Merged Order Changed Refund Sent Cancelled Cancelled (Non-Paying Buyer) Cancelled (Item Unavailable) Cancelled (Per Buyer's Request) Cancelled (Refund Received) _(e.g. `1-1`)_
- `created_after` (string, optional) — Only show orders created after this ISO 8601 timestamp. _(e.g. `2019-06-24T20:58:58Z`)_
- `created_before` (string, optional) — Only show orders created before this ISO 8601 timestamp. _(e.g. `2019-06-24T20:58:58Z`)_
- `archived` (boolean, optional) — Only show orders with a specific archived status. If no key is provided, both statuses are returned. Valid archived keys are: true false _(e.g. `true`)_
- `sort` (string, optional) — Sort items by this field (see below). Valid sort keys are: id buyer created status last_activity _(e.g. `1-1`)_
- `sort_order` (string, optional) — Sort items in a particular order (one of asc, desc) _(e.g. `1-1`)_
...
**Parameters:**
- `order_id` (string, required) — The ID of the order you are fetching _(e.g. `1-1`)_
...
**Parameters:**
- `order_id` (string, required) — The ID of the order you are fetching _(e.g. `1-1`)_
- `message` (string, optional) _(e.g. `hello world`)_
- `status` (string, optional) _(e.g. `New Order`)_
...
**Parameters:**
- `price` (number, optional) — The price to calculate a fee from _(e.g. `10.00`)_
...
**Parameters:**
- `price` (number, optional) — The price to calculate a fee from _(e.g. `10.00`)_
- `currency` (string, optional) — Defaults to USD. Must be one of the following: USD GBP EUR CAD AUD JPY CHF MXN BRL NZD SEK ZAR _(e.g. `USD`)_
...
**Parameters:**
- `release_id` (number, required) — The release ID to calculate a price from. _(e.g. `1`)_
...
**Parameters:**
- `release_id` (number, required) — The release ID whose stats are desired _(e.g. `1`)_
- `curr_abbr` (string, optional) — Currency for marketplace data. Defaults to the authenticated users currency. Must be one of the following: USD GBP EUR CAD AUD JPY CHF MXN BRL NZD SEK ZAR _(e.g. `USD`)_
